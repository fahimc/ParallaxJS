var Parallax = {
	...
	checkChildOnScreen : function() {
		...
		if( document.body.scrollTop<=this.previousTop)dir="up";
		...
		if( document.body.scrollTop>this.previousTop)
		{
		...
		this.previousTop = document.body.scrollTop;
		...
		if (this.children[nextIndex] && this.children[nextIndex].id) {
			...
			this.excuteCallbacks(childId, this.eventNames.viewIn, percentage);
				...
				console.log("moveIn index",nextIndex);
				console.log("moveIn currentIndex",this.currentIndex);
	...
	calculateMoveOut:function(dir)
	{
		...
		if( document.body.scrollTop>this.previousTop)
		{
			...
			if(this.children.length-1<=this.currentIndex)this.currentIndex=this.children.length-1;
		}
		console.log("calculateMoveOut",this.currentIndex );
		...
		for (var name in this.movers) {
			...
			if (obj.holder == holderId) {
				...
				for (var att in endPoints) {
					...
					var diff = Math.abs(endval-startval);
					
					var finalVal = (endval * ((percent / 100)))+ "px";
					console.log("------------------------");
					console.log("moveOut index",this.currentIndex);
					console.log("moveOut pecent",percent);
					console.log("moveOut endval",endval);
					console.log("moveOut diff",diff);
					console.log("moveOut ratio",(percent / 100));
					console.log("moveOut new val",diff *(percent / 100));
					console.log("moveOut finalVal",finalVal);
					console.log("moveOut dir",dir);
		...
		for (var name in this.movers) {
			...
			if (obj.holder == holderId) {
				...
				for (var att in endPoints) {
					...
					console.log("moveIn pecent",percent);
					console.log("moveIn endval",endval);
					console.log("moveIn diff",diff);
					console.log("moveIn ratio",(percent / 100));
					console.log("moveIn new val",diff *(percent / 100));
					console.log("moveIn finalVal",finalVal);
					console.log("moveIn this.previousIndex",this.previousIndex,this.currentIndex);
					if (dir=="up")return;
					if (!window.TweenLite) {
						elem.style[att] = finalVal;
					} else {
	...
	moveChildren : function(holderId, percent, dir, way) {
		...
		for (var name in this.movers) {
			...
			if (obj.holder == holderId) {
				...
				for (var att in endPoints) {
					...
					if (Number(this.getStyle(elem, att).replace("px", "")) >= startval + (diff * per) && dir == "down" && way=="out")
						return;
					if (Number(this.getStyle(elem, att).replace("px", "")) <= startval + (diff * per) && dir == "up")
						return;
					if (!window.TweenLite) {
						elem.style[att] = finalVal;
					} else {
	...
	navigateTo : function(index) {
		...
		if (!window.TweenLite) {
			...
			onComplete:function(){this.scrollDir=null},onCompleteScope:this});
		}
//this.scrollDir=null;
	},
	onScroll : function() {
